...
// .wrangler/tmp/bundle-b5W4py/checked-fetch.js
...
// src/routes/chat.ts
async function callCerebras(apiKey, model, messages, temperature, maxTokens) {
...
async function callGrok(apiKey, model, messages, temperature, maxTokens) {
  const supportedModels = [
...
function buildNsfwTagsFromState(clothingState, poseState, actionState, nsfwLevel) {
  let positive = "";
  let negative = "";
...
__name(buildNsfwTagsFromState, "buildNsfwTagsFromState");
function extractPngFromZip(zipData) {
...
__name(extractPngFromZip, "extractPngFromZip");
var imageRoute = new Hono2();
imageRoute.post("/api/image/generate", async (c) => {
  try {
    ...
    if (!response.ok) {
      const error = await response.text();
      return c.json({ success: false, error: `NovelAI API error: ${error}` }, response.status);
    }
...
imageRoute.post("/api/image/judge", async (c) => {
  try {
    ...
    const historyText = recentHistory.map((m) => `${m.role === "user" ? "\u30E6\u30FC\u30B6\u30FC" : characterName}: ${m.content}`).join("\n");
    const previousStateText = previousImageState ? `\u524D\u56DE\u306E\u30A4\u30E9\u30B9\u30C8\u72B6\u614B: \u670D\u88C5=${previousImageState.clothingState || "\u4E0D\u660E"}, \u4F53\u52E2=${previousImageState.poseState || "\u4E0D\u660E"}` : "";
...
app.use("*", cors({
  origin: /* @__PURE__ */ __name((origin, c) => {
    ...
    if (c.env.CORS_ORIGIN) {
      return origin === c.env.CORS_ORIGIN ? origin : void 0;
    }
    return "*";
  }, "origin"),
  credentials: true
...
// .wrangler/tmp/bundle-b5W4py/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-b5W4py/middleware-loader.entry.ts
